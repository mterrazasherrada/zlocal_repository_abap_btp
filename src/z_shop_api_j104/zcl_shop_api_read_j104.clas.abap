CLASS zcl_shop_api_read_j104 DEFINITION
 PUBLIC
 CREATE PUBLIC .
  PUBLIC SECTION.
...
CLASS ZCL_SHOP_API_READ_J104 IMPLEMENTATION.
  ...
  METHOD if_http_service_extension~handle_request.
    ...
    TRY.
       ...
       DATA(lo_destination) = cl_http_destination_provider=>create_by_comm_arrangement(
                                    comm_scenario               = 'Z_SHOP_SCENARIO_OUTBOUND_J104'
                                    service_id                  = 'Z_SHOP_API_READ_OBS_J104_REST' ).

       lo_http_client = cl_web_http_client_manager=>create_by_http_destination( lo_destination ).
       lo_client_proxy = cl_web_odata_client_factory=>create_v2_remote_proxy(
                                  EXPORTING
                                    iv_service_definition_name  = 'Z_SHOP_API_SCM_J104'
                                    io_http_client              = lo_http_client
                                    iv_relative_service_root    = '' ).
        " Set entity key
        ls_entity_key = VALUE #(
        order_uuid = 'DF0B7D5455421EEE87FC071D41405A2D' ).
